...
const OLD_SLUG = "daven23";
const NEW_NAME = "daven3";
...
async function main() {
  ...
  if (!player) {
    console.log(`[rename-daven23] no player with slug "${OLD_SLUG}"`);
    return;
  }
  ...
  for (const from of [oldName, OLD_SLUG, "daven23"]) {
  ...
  console.log("[rename-daven23] done:", verify);
}

main().catch((err) => {
  console.error("[rename-daven23] failed:", err);
  process.exit(1);
});
